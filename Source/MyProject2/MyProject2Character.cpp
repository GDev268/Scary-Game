...
AMyProject2Character::AMyProject2Character()
{
	...
	HoldingComponent->AddRelativeLocation(FVector(0.2f, 1000.0f, -10.6f));
...
void AMyProject2Character::Tick(float DeltaSeconds)
{
	...
	if (!bHoldingItem)
	{
		if (GetWorld()->LineTraceSingleByChannel(Hit, Start, End, ECC_Visibility, DefaultComponentQueryParams, DefaultResponseParam))
		{
			...
			else if (Hit.GetActor()->GetClass()->IsChildOf(ADoor::StaticClass()))
			{
...
void AMyProject2Character::ToggleDoor()
{
	FVector NewVector = FirstPersonCameraComponent->GetForwardVector();

	if (CurrentDoor)
	{
		CurrentDoor->ToggleDoor(NewV\ector);
	}
}

void AMyProject2Character::ToggleItemPickup()
{
	if (CurrentItem) {
...
void AMyProject2Character::OnAction()
{
	switch (typeID)
	{
		case 1:
...
void AMyProject2Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Pick", IE_Pressed, this, &AMyProject2Character::ToggleItemPickup);
}
